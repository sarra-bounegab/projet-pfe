...
<div id="modal-taches" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
    <div class="bg-white shadow-2xl rounded-2xl w-full max-w-xl p-6 relative">
        ...
        <form id="formRapport" method="POST" action="{{ route('rapports.store') }}">
            ...
            <div class="mb-4">
                <label class="block text-gray-700 font-semibold mb-2">Contenu du rapport :</label>
                <textarea name="contenu" id="contenu" class="border rounded-lg px-4 py-2 w-full focus:ring focus:ring-blue-200" rows="3" placeholder="Décrire l'intervention..."></textarea>
            </div>
            ...
            <div class="flex items-center gap-2 mb-4">
                <input type="text" id="nouvelle-tache" class="border rounded-lg px-4 py-2 w-full focus:ring focus:ring-green-200" placeholder="Ajouter une nouvelle tâche...">
                <button type="button" onclick="ajouterTache()" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg transition">Ajouter</button>
            </div>

            <div class="flex justify-between mt-6">
                <button type="button" onclick="fermerModalTaches()" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-lg">Annuler</button>
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Enregistrer</button>
            </div>
        </form>

      
        <button onclick="fermerModalTaches()" class="absolute top-3 right-3 text-gray-500 hover:text-red-500 text-xl">&times;</button>
    </div>
</div>
    ...
    fetch(`/get-taches?intervention_id=${id}`)
        ...
        .then(data => {
            ...
            if (data.success && data.taches.length > 0) {
                data.taches.forEach(tache => afficherTache(tache.description));
            } else {
...
function ajouterTache() {
    ...
    if (tacheTexte !== "") {
        ...
        labelTache.classList.add('bg-blue-500', 'text-white', 'rounded-full', 'px-3', 'py-1', 'text-sm', 'font-semibold');
...
function reNumeroTaches() {
    let labels = document.querySelectorAll('#taches-container span.bg-blue-500');
    compteurTaches = 0;
    ...
    function afficherTache(description) {
        let container = document.getElementById('taches-container');

        let divTache = document.createElement('div');
        divTache.classList.add('flex', 'justify-between', 'items-center', 'p-2', 'border', 'rounded-lg');
        ...
        btnSupprimer.classList.add('text-red-500', 'text-sm');
        btnSupprimer.onclick = function() {
            container.removeChild(divTache);
        };
